...
async def batch_predict(data: List[dict], model_and_vectorizer = Depends(get_model_and_components)):
    ...
    predictions = [
        {
            'prediction_for_date': data[i]['date'],
            'decision': bool(decision[i]),
            'probability': float(round(y_pred[i], 3)),
            'description': (
                f"It will be rain or drizzle during {data[i]['date']}. Bring your umbrella"
                if decision[i]
                else f"It will be not either rain or drizzle during {data[i]['date']}. Have a nice day"
            )
        }
